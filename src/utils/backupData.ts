...
export const setDataMonitoring = (dataToren: string) => {
  db.setItem(`dataTorenAir-${getIdBangunan()}`, dataToren);
};
...
export const setDataListrik = (dataListrik: string) => {
  db.setItem(`dataListrik-${getIdBangunan()}`, dataListrik);
};
...
export const getDataTorenAir = () => {
  const torenAir = db.getItem(`dataTorenAir-${getIdBangunan()}`);
  return torenAir !== null ? JSON.parse(torenAir) : null;
};

export const getDataListrik = () => {
  const dataListrik = db.getItem(`dataListrik-${getIdBangunan()}`);
